module Helpers
	...
	def load_json_from_fixture(filename)
		File.read("#{Kountagious.root}/spec/fixtures/#{filename}")
	end
	...
	def stub_endpoints
    # auth
    stub_request(:post, "https://api.kounta.com/v1/token.json").to_return(body: load_json_from_fixture('token.json'), headers: endpoint_headers)
